...
class NodeBase:
    ...
    def treat_action_node(node: XMLTree.Element,
        ...
        match node.tag:
            case "dir":
                dir_node.DirNode.treat_dir_node(node, temgen, tree_info)
            case "file":
                file_node.FileNode.treat_file_node(node, temgen, tree_info)
            case "if":
                if_node.IfNode.treat_if_node(node, temgen, tree_info)
            case "match":
                match_node.MatchNode.treat_match_node(node, temgen, tree_info)
            case _:
    ...
    def treat_action_children_nodes_of(node: XMLTree.Element,
        ...
        for child_node in node:
            NodeBase.treat_action_node(child_node, temgen, tree_info)
