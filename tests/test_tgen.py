...
class TestTemgenProgram(TestTemgenProgramBase):
    def test__bad_dirtree__bad_root_node_name__err(self):
        try:
            self._test_temgen_file("bad_root_node_name")
            self.fail()
    ...
    def test__if_fdirtree__invalid_two_then__exception(self):
        try:
            ...
            in_str = f'{output_root_dir}\nyes'
            self._test_temgen_file(output_root_dir, stdin_str=in_str)
            self.fail()
    ...
    def test__trivial_temgen__bad_format_str__exception(self):
        try:
            self._run_generated_trivial_temgen_file("bad_format_str", file_contents="{whut")
            self.fail()
        except ValueError as err:
            self.assertEqual(str(err), "expected '}' before end of string")

    def test__trivial_temgen__unknown_var__exception(self):
        try:
            self._run_generated_trivial_temgen_file("unknown_var", file_contents="{unknown_var}")
            self.fail()
        except KeyError:
            pass
    ...
    def test__file_fdirtree__format_raw__ok(self):
        ...
        in_str = f'{output_root_dir}\nraw\nunused_message'
        self._test_temgen_file("file_fdirtree__valid_format", project_root_dir=output_root_dir, stdin_str=in_str)

    def test__file_fdirtree__copy_raw__ok(self):
        output_root_dir = "file_fdirtree__copy_raw"
        in_str = f'{output_root_dir}\nfruits.txt\nraw\nPeer'
        self._test_temgen_file("file_fdirtree__copy", project_root_dir=output_root_dir, stdin_str=in_str)
    ...
    def test__trivial_fdirtree__builtin_CURRENT_SOURCE_DIR__ok(self):
        ...
        file_contents_dict = self._run_generated_trivial_temgen_file(project_root_dir, file_contents=f_contents)
    ...
    def test__file_template__local_xml__ok(self):
        ...
        in_str = f'{output_root_dir}\ninput/templates\ntemfile-1.0.0.xml\nobject.txt\nsword'
        self._test_temgen_file("file_template__valid", project_root_dir=output_root_dir, stdin_str=in_str)

    def test__file_template__local_xml_ver__warning(self):
        output_root_dir = "file_template__valid_local_xml_ver"
        in_str = f'{output_root_dir}\ninput/templates\ntemfile-1.0.0.xml\n1.0.0\nobject.txt\nsword'
        self._test_temgen_file("file_template__valid_ver", project_root_dir=output_root_dir, stdin_str=in_str)
    ...
    def test__vars_rand_value__all__ok(self):
        ...
        in_str = f"{output_root_dir}"
        self._test_temgen_file("vars_rand_value", project_root_dir=output_root_dir, stdin_str=in_str)

    def test__builtins__template__ok(self):
        project_root_dir = "builtins__template"
        in_str = f"input/templates/template_builtins.xml\n{project_root_dir}"
        self._run_temgen_file("dir_template__builtins", stdin_str=in_str)
